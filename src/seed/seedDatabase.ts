...
import createExampleUser from './dataCreation/createExampleUser';
...
const seedDatabase = async () => {
  ...
  await createExampleUser('example', 'password');

  const adminUser = await createExampleUser('admin', 'password');
  const allUsers = await Promise.all(userPromises);
  const beerTypes = await generateBeerTypes(adminUser);

  logger.info('Saving all users.');

  generateBreweryData(400).forEach((rawBreweryData) => {
